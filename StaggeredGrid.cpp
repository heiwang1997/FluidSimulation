...
void StaggeredGrid::test() {
	//std::set<IndexDistanceWrapper> priorityQueue;
	//priorityQueue.insert(IndexDistanceWrapper(1, 1.0));
	//priorityQueue.insert(IndexDistanceWrapper(2, 1.3));
	//cout << priorityQueue.size() << priorityQueue.begin()->index;
	//return;
	...
	// addWaterCube(5);
	setBorderType(cellType, resX, resY, resZ, SOLID);
	setBorderValue(velocityX, resX + 1, resY, resZ, 0.0f);
	for (int i = 0; i < resX; i++) {
		for (int j = 0; j < resY; j++) {
			for (int k = 0; k < resZ; k++) {
				int index = getIndex(i, j, k, resX, resY, resZ);
				// signedDistanceField[index] = sin(0.1*i + sin(0.2*j));
				signedDistanceField[index] = ((i > 30 && i < 70) ? 1 : -1);
			}
		}
	}
	for (int i = 2; i < resX - 1; i++) {
		for (int j = 1; j < resY - 1; j++) {
			for (int k = 1; k < resZ - 1; k++) {
				int index = getIndex(i, j, k, resX + 1, resY, resZ);
				velocityX[index] = i - 30;
			}
		}
	}
	real *img = new real[totalCells];
	computeSignedDistanceField(signedDistanceField);
	for (int i = 0; i < totalCells; i++) {
		if (levelSetInfo[i] == IMMEDIATE_NEIGHBOR) {
			img[i] = 1;
		}
		if (levelSetInfo[i] == UNKNOWN) {
			img[i] = 0;
		}
		if (levelSetInfo[i] == KNOWN) {
			img[i] = -1;
		}
		assert(false);
	}
	...
	cout << vx << ", " << vy << ", " << vz << endl;
	getAveragedVelocityAt(5.5, 6, 3, vx, vy, vz);
	cout << vx << ", " << vy << ", " << vz << endl;
	dumpSlicePreview(6, signedDistanceField, 3, 1.0);
	resX++;
	dumpSlicePreview(7, velocityX, 3, 0.02);
	resX--;
	...
	for (int i = 0; i < totalCells; i++) {
		if (levelSetInfo[i] == IMMEDIATE_NEIGHBOR) {
			signedDistanceField[i] = 1;
		}
		if (levelSetInfo[i] == UNKNOWN) {
			signedDistanceField[i] = 0;
		}
		if (levelSetInfo[i] == KNOWN) {
			signedDistanceField[i] = -1;
		}
	...
	dumpSlicePreview(9, signedDistanceField, 3, 1);
}
